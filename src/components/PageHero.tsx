    ...
    <section className="wash-warm relative overflow-hidden border-b border-black/5">
      ...
      <Container className="relative py-10 md:py-14">
        ...
        {stats && stats.length > 0 && (
          <div className="mt-8 overflow-hidden rounded-lg border border-black/5 bg-white text-navy shadow-sm sm:overflow-visible sm:rounded-none sm:border-none sm:bg-transparent sm:shadow-none sm:grid sm:grid-cols-3 sm:gap-4">
            {stats.map((stat, i) => (
              <div
                key={stat.label}
                className={`flex items-center justify-between gap-4 px-5 py-4 sm:block sm:rounded-lg sm:border sm:border-black/5 sm:bg-white sm:p-5 sm:shadow-sm ${
                  i !== stats.length - 1 ? "border-b border-black/5 sm:border-b-0" : ""
                }`}
              >
                <span className="text-sm text-muted sm:hidden">{stat.label}</span>
                <span className="font-mono font-bold sm:block">{stat.value}</span>
